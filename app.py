...
# UPDATED: เปลี่ยนไปใช้ st.markdown() ซึ่งเข้ากันได้กับทุกเวอร์ชัน
...
def process_text_step1(text):
    ...
    # 1. ลบวงเล็บที่ไม่ใช่รหัสสาขา (อนุญาตให้เลข 4 หรือ 5 หลักอยู่ในวงเล็บได้)
    ...
    # 2. เติมวงเล็บให้เลข 5 หลักที่ยังไม่มี
...
def process_data(df):
    try:
        ...
        # ล้างเฉพาะคอลัมน์ชื่อบริษัท (คอลัมน์ที่ 9)
        df_cleaned[9] = df_cleaned[9].apply(process_text_step1)
        
        # แก้ไข applymap เป็น map เนื่องจากถูกยกเลิกใน Pandas 3.0+
        df_cleaned = df_cleaned.map(process_text_step2)
...
if st.button("🚀 ประมวลผลไฟล์"):
    if uploaded_file:
        try:
            ...
            if zip_file:
                st.success("✅ ประมวลผลสำเร็จ! ดาวน์โหลดไฟล์ ZIP ได้ที่ปุ่มด้านล่าง")
                st.download_button(
        ...
        st.warning("⚠️ กรุณาอัปโหลดไฟล์ก่อนกดประมวลผล")
